...
`specs/006-k8s-host-remote-access/plan.md`（特性：K8S Host 远程接入与配置热生效——① SSH 引导接入（波1）：用户只配 master IP+root+密码，网关 SSH 登 master 取 `/etc/kubernetes/admin.conf`（k3s `/etc/rancher/k3s/k3s.yaml`）构造 fabric8 client，免用户处理 K8S 鉴权；K8sHost 加 `ssh` 子段（含 `server-override` + `insecure-skip-tls-verify` 兜底），与 `kubeconfig` 互斥；SSH 库 sshj；② 全配置热生效（波2）：`config/k8s-hosts.yaml` + `K8sHostsWatcher`（仿 BackendConfigWatcher）+ `K8sHostStore`（host 生命周期 diff，建/关/重建）+ k8s 全局参数下次 ensure 读新值；三触发源统一热重载管道；③ portal 管理（波3）：`/admin/k8s-hosts` CRUD + root 密码 AES-GCM 加密（密钥 `ARTHAS_GATEWAY_SECRET`）+ 前端 `/k8s-hosts`；④ 显示增强 + bug 修复（波4）：`BackendDto` 加 K8S 来源字段 + 修 ensure 后 portal 不显示 bug（`BackendConfigWatcher` compose 异常吞咽 + 前端无自动刷新）；零 gateway-core K8S/SSH 依赖不变（ArchUnit 守护 sshj），003/005 既有契约全部不破）。
配套产出：`research.md`（R1–R10）、`data-model.md`（K8sHost.ssh/SshBootstrap/SshKubeconfigFetcher/K8sHostStore/HostEntry/K8sHostsWatcher/K8sHostSecretCipher/K8sHostAdmin DTO/BackendDto 增量）、`contracts/host-remote-access-invariants.md`（INV-SSH-* + INV-HOT-* + INV-PORTAL-K8S-* + INV-DISP-* + INV-BOUNDARY-3）、`quickstart.md`；规格见 `spec.md`；设计决策见 `docs/superpowers/specs/2026-07-13-k8s-host-remote-access-design.md`。
上一特性基线（回归对照）：`specs/005-k8s-orchestration-iteration/`（更早 `specs/004-portal-backend-management/`、`specs/003-k8s-arthas-mcp-launch/`、`specs/002-code-review-remediation/`、`specs/001-arthas-mcp-gateway/`）。
<!-- SPECKIT END -->
